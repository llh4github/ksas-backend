package io.github.llh4github.ksas.common.consts

object RolePermConst {
    const val QUERY_SINGLE = "auth:role:view:id"
    const val QUERY_PAGE = "auth:role:view:page"
    const val ADD_DATA = "auth:role:add"
    const val UPDATE_DATA = "auth:role:update"
}

object UserPermConst {
    const val QUERY_SINGLE = "auth:user:view:id"
    const val QUERY_PAGE = "auth:user:view:page"
    const val QUERY_PAGE_ACTIVE = "auth:user:view:page:active"
    const val ADD_DATA = "auth:user:add"
    const val UPDATE_DATA = "auth:user:update"
}

object PermissionPermConst {
    const val QUERY_SINGLE = "auth:permission:view:id"
    const val QUERY_PAGE = "auth:permission:view:page"
    const val ADD_DATA = "auth:permission:add"
    const val UPDATE_DATA = "auth:permission:update"
}


